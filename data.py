...
class SignalGWDataSet(pl.LightningDataModule):
    def __init__(
        self,
        ...
        time_duration: float,
        prior_func: Optional[Callable] = nonspin_bbh_chirp_mass_q_parameter_sampler,
    ) -> None:
        super().__init__()
        ...
        # Remove prior initialization if not needed
        if self.prior_func:
            self.prior = self.prior_func()

    def setup(self, stage: str) -> None:
        # Use TensorDataset to create a dataset from strain data
        self.dataset = TensorDataset(
            torch.tensor(self.strain_data, dtype=torch.float32), 
            torch.zeros(self.strain_data.shape[0], 8, dtype=torch.float32)  # Placeholder for parameters
        )
    ...
    def __next__(self) -> Tuple[torch.Tensor, torch.Tensor]:
        # Return strain data and a placeholder for parameters
        X, parameters = super().__next__()  # This will now unpack correctly
        return X, parameters
...
class SignalDataSet(pl.LightningDataModule):
    ...
    def setup(self, stage: str) -> None:
        ...
        if Path("whitener.pt").exists():
            ...
            torch.save(self.preprocessor.whitener.state_dict(), "whitener.pt")  # ±£´æ whitener
