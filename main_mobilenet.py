...
from mlpe.architectures.embeddings import ResNet, QuantizedResNet, Simple1DCNN, MobileNet1D, MobileNet1DV2, MobileNetV3_1D
...
def main():
    ...
    '''
    embedding = MobileNet1DV2(
        num_ifos=n_ifos,
        context_dim=mobilenetv2_context_dim,
        layers=mobilenetv2_layers,
        expand_ratios=mobilenetv2_expand_ratios,
        initial_channels=mobilenetv2_initial_channels,
        width_mult=mobilenetv2_width_mult
    )
    '''
    embedding = MobileNetV3_1D(num_ifos=n_ifos, context_dim=resnet_context_dim, version="small")#Simple1DCNN(num_ifos=n_ifos, context_dim=resnet_context_dim)
    '''
    embedding = ResNet(
        (n_ifos, strain_dim),
    ...
    trainer = Trainer(
        ...
        logger=logger,
        gradient_clip_val=10.0,#10.0,
    )
    start_time = time.time()
    trainer.fit(model=flow_obj, datamodule=sig_dat)
    end_time = time.time()
    print(f"Training time: {end_time - start_time} seconds")
    torch.save(flow_obj.state_dict(), 'org_mobile1d_epoch10.pth')
